...
export function Hero() {
  ...
  const motionProps = shouldReduceMotion ? {} : { initial: 'hidden', animate: 'visible', variants: container }
    ...
    <section id="home" className="relative min-h-screen flex items-center pt-24 pb-16">
      <div className="max-w-content w-full mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-16 items-center">
        <motion.div {...motionProps} className="flex flex-col gap-6">
          ...
          <motion.h1 {...itemMotionProps} className="text-5xl sm:text-6xl lg:text-[4.5rem] font-semibold tracking-tight leading-[1.05]">
            {siteConfig.name}
          </motion.h1>

          <motion.p {...itemMotionProps} className="text-lg sm:text-xl text-accent font-medium font-display">
            {siteConfig.role}
          </motion.p>

          <motion.p {...itemMotionProps} className="text-xl sm:text-2xl text-foreground/90 max-w-xl text-balance">
            {siteConfig.heroStatement}
          </motion.p>

          <motion.p {...itemMotionProps} className="text-muted text-base sm:text-lg max-w-xl leading-relaxed">
            {siteConfig.heroParagraph}
          </motion.p>

          <motion.div {...itemMotionProps} className="flex flex-wrap items-center gap-4 mt-2">
            <a
              href="#projects"
          ...
          <motion.div {...itemMotionProps} className="flex items-center gap-3 mt-2">
